...
BaseTransport.prototype.send = function (fcode, extra, next) {
	...
	const retry = extra.retry || this.retry;
	...
	if (this.mutex) {
		...
		this.mutex.lock(function (unlock) {
			...
			transport.retrySend(transport.wrap(pdu[fcode].Request.build.apply(pdu[fcode].Request, data), extra, done), transport.retries, /*transport.retry*/ retry, done);
		})
	} else {
		this.retrySend(this.wrap(pdu[fcode].Request.build.apply(pdu[fcode].Request, data), extra, next), this.retries, /*this.retry*/ retry, next);
	}
};
