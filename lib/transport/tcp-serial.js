...
Transport.prototype.wrap = function (pdu, options, next) {
    ...
    // var unitId = (typeof options.unitId != 'undefined' ? options.unitId : 0xFE);
    // var data = buffer.alloc(pdu.length + 3);
    // data.writeUInt8(unitId, 0);
    ...
    if (options.crc != false) {
        data.writeUInt16LE(Transport.prototype.crc16(data.slice(0, data.length - 2)), pdu.length + 1);
    }
    return data;
}
...
Transport.prototype.unwrap = function (data) {
	...
	var unwrapped = {
		...
		crc          : data.readUInt16LE(data.length - 2),
		expected_crc : Transport.prototype.crc16(data.slice(0, data.length - 2)),
		pdu          : data.slice(1, data.length - 2),
		stream       : this.stream
	};
...
Transport.prototype.crc16 = function (buf) {
    var crc = 0xFFFF;
